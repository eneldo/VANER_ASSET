...
down_revision: str | Sequence[str] | None = "f284acc97939"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    op.create_table(
        ...
        sa.Column(
            "prioridad", sa.String(20), server_default="EMERGENCIA", nullable=False
        ),
        ...
        sa.ForeignKeyConstraint(
            ["solicitante_id"], ["usuarios.id"], ondelete="RESTRICT"
        ),
        sa.ForeignKeyConstraint(
        ...
        sa.CheckConstraint(
            "prioridad IN ('ALTA', 'CRITICA', 'EMERGENCIA')",
            name="ck_solicitud_prioridad",
        ),
        sa.CheckConstraint(
            "estado IN ('NUEVA', 'EN_REVISION', 'APROBADA', 'CONVERTIDA_OT', 'RECHAZADA', 'CERRADA')",
            name="ck_solicitud_estado",
        ),
    )
    op.create_index(
        "ix_solicitudes_tenant_estado",
        "solicitudes_correctivas",
        ["empresa_id", "estado"],
    )
    op.create_index(
        "ix_solicitudes_tenant_fecha",
        "solicitudes_correctivas",

...
def production_settings(**overrides):
    values = {
        ...
        "DEBUG": False,
        "DATABASE_URL": "postgresql://sga_app:password@postgres:5432/sga",
...
class ProductionSecurityTests(unittest.TestCase):
    ...
    def test_rechaza_frontend_sin_https(self):
        with self.assertRaises(ValueError):
            production_settings(
                FRONTEND_URL="http://asset.empresaxyz.com"
            )
    ...
    def test_requiere_rol_backup_dedicado(self):
        ...
        # Debe existir una URL de backup.
        with self.assertRaises(ValueError):
            production_settings(
                BACKUP_DATABASE_URL=None
            )

        # No puede reutilizar el mismo usuario de la aplicación.
        with self.assertRaises(ValueError):
            production_settings(
                BACKUP_DATABASE_URL=(
                    "postgresql://sga_app:"
                    "otra-password@postgres:5432/sga"
                )
            )

        # No se permite un rol propietario/administrativo.
        with self.assertRaises(ValueError):
            production_settings(
                BACKUP_DATABASE_URL=(
                    "postgresql://sga_owner:"
                    "otra-password@postgres:5432/sga"
                )
            )

        # El rol histórico/incorrecto tampoco debe aceptarse.
        with self.assertRaises(ValueError):
            production_settings(
                BACKUP_DATABASE_URL=(
                    "postgresql://vaner_backup:"
                    "otra-password@postgres:5432/sga"
                )
            )

        # El rol válido de backup debe ser sga_backup.
        configured = production_settings(
            BACKUP_DATABASE_URL=(
                "postgresql://sga_backup:"
                "otra-password@postgres:5432/sga"
            )
        )
        ...
        self.assertIn(
            "sga_backup",
            configured.BACKUP_DATABASE_URL,
        )
